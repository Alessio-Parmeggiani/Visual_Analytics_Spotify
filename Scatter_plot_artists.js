...
function ScatterPlotMain(data, margin, width, height, svg) {
    ...
    if (view_artist) {
        ...
        for(var i=0;i<songByArtists_.length;i++){
            ...
            //name of artist as additional info
            songByArtists["info"].push(songByArtists_[i][0])
        }
    ...
    */
    ...
    .append("circle")
        ...
        .on("click", function(d) {
            ...
            if (view_artist) {
                named_data=false
                console.log("selected artist:",d.originalTarget.__data__[3])
            }
            else  console.log("selected song:",d.originalTarget.__data__[2])
            updateRadialPlot(d.originalTarget.__data__[2],named=named_data)
        })
